// src/heal/gate.ts — PURE, unit-tested safety functions for `praktor heal`. Healing MERGES the base
// branch into the PR branch and pushes normally (NEVER a force-push — a non-fast-forward is simply
// rejected, so a concurrent push can't be clobbered). Every dangerous decision (is this PR ours? may
// we push? have we looped too many times?) is a pure predicate here, tested in isolation, with NO I/O.
// The orchestration in cli/commands/heal.ts only ever ACTS on a `true` from these gates.
...
export interface PrePushState {
  ...
  mergeInProgress: boolean; // `.git/MERGE_HEAD` still present (the merge is not committed yet)
...
/**
 * Pre-push gate: push ONLY if the working tree is clean, there are ZERO conflict markers, the merge
 * is committed, the repo's checks are GREEN, and auto-merge is NOT enabled. On ANY failure the caller
 * aborts the merge, hard-resets, and escalates — it must NEVER push.
 */
export function prePushGate(state: PrePushState): PrePushDecision {
  ...
  if (state.mergeInProgress) return { push: false, reason: "merge not committed" };
